...
export const getUserTaskLists = async (req, res, next) => {
    try {
...
export const addTaskToFavorite = async (req, res, next) => {
    try {
        const taskId = req.params.id;
        const task = await Task.findById(taskId);
        if (!task) {
...
export const removeTaskFromFavorite = async (req, res, next) => {
    try {
        const taskId = req.params.id;
        const task = await Task.findById(taskId);
        if (!task) {
...
export const getInProgressTasks = async (req, res, next) => {
    try {
        const page = Number(req.query.page) || 1;
        const limit = Number(req.query.limit) || 10;
        const skip = (page - 1) * limit;
